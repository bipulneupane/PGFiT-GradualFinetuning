...
def freeze_mitb2_layers_from_block(model, block_name):
    """
    Freezes or unfreezes layers in a MiTb2-based model based on the specified block name.

    Args:
        model (torch.nn.Module): The model instance containing the MiTb2 encoder.
    ...
    Process:
        ...
        3. **Unfreeze Encoder Blocks**: Gradually unfreezes encoder blocks based on the specified `block_name`. 
           Each block includes specific layer prefixes, as defined in the `block_layers` dictionary.
        4. Prints the layers or blocks unfrozen after each step for transparency.

    Block Structure:
        - **Block 1**: Shallowest layers of the encoder.
        - **Block 2**: Deeper layers of the encoder.
        - **Block 3**: Even deeper layers of the encoder.
        - **Block 4**: Deepest layers of the encoder.
        - **Decoder**: All layers in the decoder and segmentation head.

    Example:
        >>> model = get_model_from_smp("Unet", "mitb2", None, len(CLASSES), "sigmoid")
        >>> freeze_mitb2_layers_from_block(model, "Block 2")
        Unfrozen up to decoder
        Unfrozen up to Block 2
    """

    # Dictionary to map block names to corresponding layer prefixes in MiTb2 encoder
    block_layers = {
        'Block 4': ['encoder.block4', 'encoder.norm4', 'encoder.patch_embed4'],
        'Block 3': ['encoder.block3', 'encoder.norm3', 'encoder.patch_embed3'],
        'Block 2': ['encoder.block2', 'encoder.norm2', 'encoder.patch_embed2'],
        'Block 1': ['encoder.block1', 'encoder.norm1', 'encoder.patch_embed1'],
    }
    ...
    if block_name in ['decoder', 'Block 4', 'Block 3', 'Block 2', 'Block 1']:
    ...
    if block_name in blocks_to_unfreeze:
        for block in blocks_to_unfreeze[block_name]:
            for layer_prefix in block_layers[block]:
                for name, param in model.named_parameters():
...
def fine_tune_unet_mit_b2(model_name, bb, data_type, CLASSES, BATCH_SIZE, EPOCHS, LR, optimiser, loss_func, checkpoint_path, block_name, postfix):
    ENCODER = bb
    DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')  # Change to 'mps' if using Apple M1/M2
    model_save = model_name + '-' + ENCODER
    checkpoint_save_path = './trained_models/SDA-strategies/NormalSDA-upto-'+ str(block_name.replace(' ', '')) + '-' + model_save + '-' + data_type + '-' + str(EPOCHS) + 'ep-' + str(optimiser) + '-' + str(loss_func) + '-' + str(postfix) + '.pth'
    ...
    freeze_mitb2_layers_from_block(model, block_name)
...
##### GRADUAL FINE-TUNING - UPTO BLOCK 3/1
...
def gradual_fine_tune_unet_mit_b2(model_name, bb, data_type, CLASSES, BATCH_SIZE, EPOCHS, LR, optimiser, loss_func, checkpoint_path, postfix):
    """
    Fine-tunes a U-Net model with a MiTb2 encoder up to a specified block, using a given dataset and training configuration.

    Args:
        model_name (str): Name of the U-Net model architecture.
        bb (str): Backbone encoder (e.g., 'mitb2').
        data_type (str): Dataset type specifying directory structure (e.g., 'segmentation').
        CLASSES (list): List of target class names for segmentation.
        BATCH_SIZE (int): Batch size for training and validation.
        EPOCHS (int): Number of training epochs.
        LR (float): Learning rate for the optimizer.
        optimiser (str): Name of the optimizer to use (e.g., 'adam', 'sgd').
        loss_func (str): Loss function to use (e.g., 'dice_loss', 'bce_loss').
    ...
    Process:
        ...
        2. **Model Compilation**:
            ...
            - Loads pre-trained weights from the provided checkpoint file.
        3. **Layer Freezing**:
            - Freezes or unfreezes layers up to the specified `block_name` using the `freeze_mitb2_layers_from_block` function.
        ...
        5. **Training Loop**:
            ...
            - Saves the best-performing model checkpoint based on IoU score.
            - Stops training early if no improvement is observed for 11 consecutive epochs.
        6. **Logging**:
            - Prints model configuration details, training progress, and validation metrics.

    Model Save Path:
        The trained model is saved at:
        `./trained_models/SDA-strategies/NormalSDA-upto-{block_name}-{model_name}-{bb}-{data_type}-{EPOCHS}ep-{optimiser}-{loss_func}-{postfix}.pth`

    Example:
        >>> fine_tune_unet_mit_b2(
        ...     model_name="Unet",
        ...     bb="mitb2",
    ...
    DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model_save = model_name + '-' + ENCODER
    checkpoint_save_path = './trained_models/SDA-strategies/GradualSDA-upto-Block1-' + model_save + '-' + data_type + '-' + str(EPOCHS) + 'ep-' + str(optimiser) + '-' + str(loss_func) + '-' + str(postfix) + '.pth'
    ...
    print()
    print("********************************************************************************")
    print("Task: Gradual Fine-tuning upto a Block 1 upon IoU stagnation for 5 epochs")
    ...
    phases = ['decoder', 'Block 4', 'Block 3', 'Block 2', 'Block 1'] # Choose: Upto all blocks OR Upto Block 3 ??
    ...
    for phase in phases:
        ...
        freeze_mitb2_layers_from_block(model, phase)
        ...
        while True:
            ...
            # **Move to the next phase after 5 epochs (for decoder phase)**
            #if phase == 'decoder' and global_epoch >= phase_max_epochs:
                #print(f"Completed 5 epochs of decoder-only fine-tuning. Moving to the next phase (bottleneck).")
                #break
...
def bonai_multilabel_gradual_fine_tune_unet_mit_b2(model_name, bb, data_type, CLASSES, BATCH_SIZE, EPOCHS, LR, optimiser, loss_func, checkpoint_path, postfix):
    """
    Gradually fine-tunes a multi-label U-Net model with a MiTb2 encoder for footprint and roof segmentation,
    progressively unfreezing layers up to "Block 1" upon IoU stagnation for 5 epochs.

    Args:
        model_name (str): Name of the U-Net model architecture.
        bb (str): Backbone encoder (e.g., 'mitb2').
        data_type (str): Dataset type specifying directory structure (e.g., 'segmentation').
        CLASSES (list): List of target class names for multi-label segmentation (e.g., ['footprint', 'roof']).
        BATCH_SIZE (int): Batch size for training and validation.
        EPOCHS (int): Total number of epochs for all phases.
        LR (float): Initial learning rate for the optimizer.
        optimiser (str): Name of the optimizer to use (e.g., 'adam', 'sgd').
        loss_func (str): Loss function to use (e.g., 'dice_loss', 'jaccard_loss').
    ...
    Process:
        ...
        3. **Gradual Fine-Tuning**:
            - Implements a multi-phase training approach with layer-wise unfreezing:
              - Phases: ['decoder', 'Block 4', 'Block 3', 'Block 2', 'Block 1'].
            - Each phase continues until IoU stagnates for 5 consecutive epochs.
            - Adjusts patience and resets counters for phase-specific stagnation.
        4. **Training Loop**:
        ...
        5. **Logging**:
            - Prints model configuration details, training progress, phase transitions, and validation metrics (Precision, Recall, IoU, F1, and Loss).

    Model Save Path:
        The trained model is saved at:
        `./trained_models/SDA-strategies/GradualSDA-upto-Block1-{model_name}-{bb}-{data_type}-{EPOCHS}ep-{optimiser}-{loss_func}-{postfix}.pth`

    Example:
        >>> bonai_multilabel_gradual_fine_tune_unet_mit_b2(
        ...     model_name="Unet",
        ...     bb="mitb2",
        ...     data_type="segmentation",
    ...
    print()
    print("********************************************************************************")
    print("Task: Multilabel (footprint, roof) - Gradual Fine-tuning upto a Block 1 upon IoU stagnation for 5 epochs")
...
def gradual_fine_tune_disclr_unet_mit_b2(model_name, bb, data_type, CLASSES, BATCH_SIZE, EPOCHS, LR, optimiser, loss_func, checkpoint_path, postfix):
    """
    ...
    Gradually fine-tunes a U-Net model with a MiTb2 encoder using discriminative learning rate (LR) reduction 
    and progressive unfreezing of layers up to "Block 1" upon IoU stagnation for 5 epochs.
    ...
    Process:
        ...
        3. **Gradual Fine-Tuning with Discriminative LR**:
            - Implements a multi-phase training approach with layer-wise unfreezing:
              - Phases: ['decoder', 'Block 4', 'Block 3', 'Block 2', 'Block 1'].
            - Adjusts the learning rate per phase, progressively reducing it for deeper layers.
            - Each phase continues until IoU stagnates for 5 consecutive epochs.
        4. **Training Loop**:
            - Trains the model for the specified number of epochs across all phases.
            - Saves the best-performing model checkpoint based on IoU score.
    ...
    Model Save Path:
        The trained model is saved at:
        `./trained_models/SDA-strategies/DiscLR-GradualSDA-upto-Block1-{model_name}-{bb}-{data_type}-{EPOCHS}ep-{optimiser}-{loss_func}-{postfix}.pth`

    Example:
        >>> gradual_fine_tune_disclr_unet_mit_b2(
        ...     model_name="Unet",
        ...     bb="mitb2",
        ...     data_type="segmentation",
        ...     CLASSES=["building"],
    ...
    DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model_save = model_name + '-' + ENCODER
    checkpoint_save_path = './trained_models/SDA-strategies/DiscLR-GradualSDA-upto-Block1-' + model_save + '-' + data_type + '-' + str(EPOCHS) + 'ep-' + str(optimiser) + '-' + str(loss_func) + '-' + str(postfix) + '.pth'
    ...
    print()
    print("********************************************************************************")
    print("Task: Gradual Fine-tuning with Discriminative LR reduction upto a Block 1 upon IoU stagnation for 5 epochs")
...
def gradual_fine_tune_stlr_unet_mitb2(model_name, bb, data_type, CLASSES, BATCH_SIZE, EPOCHS, LR, optimiser, loss_func, checkpoint_path, postfix):
    """
    ...
    Gradually fine-tunes a U-Net model with a MiTb2 encoder using a slanted triangular learning rate (STLR) 
    schedule and progressive unfreezing of layers up to "Block 1" upon IoU stagnation for 5 epochs.
    ...
    Process:
        ...
        3. **Gradual Fine-Tuning with STLR**:
            - Implements a multi-phase training approach with layer-wise unfreezing:
              - Phases: ['decoder', 'Block 4', 'Block 3', 'Block 2', 'Block 1'].
            - Adjusts base learning rates per phase, progressively reducing it for deeper layers.
    ...
    Model Save Path:
        The trained model is saved at:
        `./trained_models/SDA-strategies/STLR-GradualSDA-upto-Block1-{model_name}-{bb}-{data_type}-{EPOCHS}ep-{optimiser}-{loss_func}-{postfix}.pth`

    Example:
        >>> gradual_fine_tune_stlr_unet_mitb2(
        ...     model_name="Unet",
        ...     bb="mitb2",
        ...     data_type="segmentation",
        ...     CLASSES=["building"],
    ...
    DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model_save = model_name + '-' + ENCODER
    checkpoint_save_path = './trained_models/SDA-strategies/STLR-GradualSDA-upto-Block1-' + model_save + '-' + data_type + '-' + str(EPOCHS) + 'ep-' + str(optimiser) + '-' + str(loss_func) + '-' + str(postfix) + '.pth'
    ...
    print("\n********************************************************************************")
    print("Task: Gradual Fine-tuning with STLR upto Block 1")
    ...
    for phase in phases:
        ...
        # Set phase-specific maximum learning rate
        if phase == 'decoder':
            base_lr = LR
        elif phase == 'Block 4':
            base_lr = LR * 0.75
        ...
        # Optimizer setup for each phase with STLR
        ...
        for epoch in range(EPOCHS):
            ...
            # Update learning rate using STLR
            current_lr = slanted_triangular_lr(epoch, base_lr, EPOCHS)  # Passing EPOCHS for STLR
...
# MiT_b2 (SegFormer b2)
#train_unet_encoders('Unet', 'mit_b2', 'WHU-256', ['building'], 16, 50, 0.0001, 'Adam', 'dice_loss', 'Pretrained-')
#train_unet_encoders('Unet', 'mit_b2', 'MELB-multires', ['building'], 16, 50, 0.0001, 'Adam', 'dice_loss', 'NoSDA-')

# BONAI dataset
#bonai_multilabel_training('Unet', 'mit_b2', 'BONAI-shape', ['footprint','roof'], 8, 50, 0.0001, 'Adam', 'dice_loss', 'NoSDA-multilabel-')
...
#pretrained_ckpt = "./trained_models/SDA-strategies/Pretrained-Unet-mit_b2-WHU-256-50ep-Adam-dice_loss.pth"

########## Normal Fine-tuning by block name
#fine_tune_unet_mit_b2('Unet', 'mit_b2', 'MELB-multires', ['building'], 16, 50, 0.00001, 'Adam', 'dice_loss', pretrained_ckpt, 'decoder', 'LR5')
#fine_tune_unet_mit_b2('Unet', 'mit_b2', 'MELB-multires', ['building'], 16, 50, 0.00001, 'Adam', 'dice_loss', pretrained_ckpt, 'Block 4', 'LR5')
#fine_tune_unet_mit_b2('Unet', 'mit_b2', 'MELB-multires', ['building'], 16, 50, 0.00001, 'Adam', 'dice_loss', pretrained_ckpt, 'Block 3', 'LR5')
#fine_tune_unet_mit_b2('Unet', 'mit_b2', 'MELB-multires', ['building'], 16, 50, 0.00001, 'Adam', 'dice_loss', pretrained_ckpt, 'Block 2', 'LR5')
#fine_tune_unet_mit_b2('Unet', 'mit_b2', 'MELB-multires', ['building'], 16, 50, 0.00001, 'Adam', 'dice_loss', pretrained_ckpt, 'Block 1', 'LR5')
...
########################## PGFiT - MELB Dataset ##########################
#gradual_fine_tune_unet_mit_b2('Unet', 'mit_b2', 'MELB-multires', ['building'], 16, 50, 0.0001, 'Adam', 'dice_loss', pretrained_ckpt, 'LR4')


######################### PGFiT - Massachusetts DATASET ##################
#gradual_fine_tune_unet_mit_b2('Unet', 'vgg19', 'Massachusetts', ['building'], 16, 50, 0.0001, 'Adam', 'dice_loss', pretrained_ckpt, 'LR4')
...
########################## DiscLR ##########################
#gradual_fine_tune_disclr_unet_mit_b2('Unet', 'mit_b2', 'MELB-multires', ['building'], 16, 50, 0.0001, 'Adam', 'dice_loss', pretrained_ckpt, 'LR4')


########################## STLR ##########################
#gradual_fine_tune_stlr_unet_mitb2('Unet', 'mit_b2', 'MELB-multires', ['building'], 16, 100, 0.0001, 'Adam', 'dice_loss', pretrained_ckpt, 'LR4')


########### Evaluate models
#evaluate_edns(pretrained_ckpt, 'Unet', 'mit_b2', 'WHU-256', 'dice_loss')
